...
figure
hold
plot((1:length(Vinterp))*deltat,Vinterp(:,1),'b')
for ii = 1:numrep
    plot([((ii-1)*writeintsteps+1):(ii*writeintsteps+1)]*deltat, Vinterp([((ii-1)*writeintsteps+1):(ii*writeintsteps+1)],1)','g--')
end
...
for ii=1:numrep % number of data writing cycles
    if ii > 1
        ...
        % estimator
        Vd = Vinterp([((ii-1)*writeintsteps+1):(ii*writeintsteps+1)],1)'; 
...
%        p2(:,ii)=plot(([((ii-1)*writeintsteps+1):50:ii*writeintsteps])*deltat,Vd(crange,1:50:writeintsteps),'--');
        p2(:,ii)=plot(([((ii-1)*writeintsteps+1):50:ii*writeintsteps])*deltat,Vinterp([((ii-1)*writeintsteps+1):50:ii*writeintsteps],crange),'--');
        clear hprime acoeff* usf Verror nerror iext V n un
    end
...
%            fiberlength = deltax*numpart; 
    reprange = (numrep-10):(numrep-1);%10:20%1:10%11:20%41:50%
