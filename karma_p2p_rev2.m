% Version for use with C.T. Kelley's nsoli.m. Single-cell simulator takes initial 
% state as input, runs for 1 BCL, and returns final state. BCL must be
% adjusted manually. Based on karma_sim_..._ctrbgen.m. Set fbon_time to 1
% for CL tests. 

% Older comments:

% This version generates data for karma_sim_..._ctrbtest.m. The latter
% computes ctrb Grammians for a single cell system. 

% Added 1/nB factor to acoeff2, even though it shouldn't have an effect for
% nB=1
% Changed theta function to smooth version to be consistent with
% linearization assumption.

% Main changes: 1) revised code to use wraparound data structure to avoid memory
% errors, 2) corrected mistakes in Jacobian code. 
...
% versions. 
% The dapd plot should also be restored.
...
function xout = karma_sim_statefbtest(x)%karma_sim(karma_test,pubyear,stimperiod)
...
writeint = bcl; % maximum array length in ms, 20 cells
...
stimstart = 0.8;%deltat; % start time for stimulation (ms)
...
stimindices = stimstartindex + [0  transperiodsteps transperiodsteps(end)+cumsum(stimperiodsteps(end)*ones(1,10))];
...
%desfile = 'desval_1cellOL_nsoli_b400';
%desfile = 'desval_1cellOL_nsoli_b800';
desfile = 'desval_1cellOL_nsoli_b800_stimstart100';
...
%                                    KIred1((numpart+1):(numpart+1))= -0.2; 
...
%                                    KIred1(60) = [-0.0002]; % Doesn't work as well as same gain at cell 40 
        ...
        %     if k > fbon_time
        %         un(:,k) = deltat*nerror(:,k)/tauN; % proportional feedback term applied to n instead of V
        %     end
        nnext = n(:,k) + deltat*karma_g(V(:,k),n(:,k))/tauN + un(:,k);
...
%    eval(['save data/' num2str(ii) ' usf Verror nerror iext V n apds apendindices dctr rctr *prev *next']);  % s iint Vx_dn Vx_up Vdn Vup
    eval(['save data/' num2str(ii) ' hprime acoeff* usf Verror nerror iext V n apds apendindices dctr rctr *prev *next']);  % s iint Vx_dn Vx_up Vdn Vup
%    eval(['save data/' num2str(ii) ' usf Verror nerror iext V n apds apendindices dctr rctr *prev *next']);  % s iint Vx_dn Vx_up Vdn Vup
    clear hprime acoeff* usf Verror nerror iext V n un 
    if ii == numrep
...
%        save data/configinfo B1 B2 stimperiod nB M epsil1 epsil2 tauN Vstar Vn Vh Vb b ch1 ch2 ch3 cc_* finaltime deltat deltax numstep numpart stimduration stimheight writeint numrep detvec fbon_time KIred1 stimindices iextd writeintsteps fiberlength
        save data/configinfo B1 B2 stimperiod nB M epsil1 epsil2 tauN Vstar Vn Vh Vb b ch0 ch1 ch2 ch3 cc_* finaltime deltat deltax numstep numpart stimduration stimheight writeint numrep detvec fbon_time KIred1 stimindices iextd writeintsteps fiberlength stimstart
%        xout = [Vnext; nnext]-x;
        xout = [Vnext; nnext];
    end
end % data write loop
...
%        p(end+1)=plot(deltat*apendindices{i}, dapd*ones(size(apendindices{i})),'b--');
