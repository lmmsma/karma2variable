...
mdeltat = -deltat; % use for backwards Euler? 
...
%            iext(2,k) = -1*Verror(2,k); 
        ...
        if numpart > 1
            if bc == 'n'
                ...
                Vnext(1) = V(1,k) + epsil1*mdeltat*(-Vx_up(k)*deltax - V(1,k)+V(2,k))/(deltax)^2 + mdeltat*karma_f(V(1,k),n(1,k),nB,M)/epsil2 ...
                    - mdeltat*cc_ext*iext(1,k) + mdeltat*cc_int*iint(1,k);
                Vnext(numpart) = V(numpart,k) + epsil1*mdeltat*(V(numpart-1,k) - V(numpart,k) + Vx_dn(k)*deltax)/(deltax)^2 + mdeltat*karma_f(V(numpart,k),n(numpart,k),nB,M)/epsil2 ...
                    - mdeltat*cc_ext*iext(numpart,k) + mdeltat*cc_int*iint(numpart,k);
            elseif bc == 'd'
                ...
                %                     - deltat*cc_ext*iext(numpart,k) + deltat*cc_int*iint(numpart,k);
                Vnext(1) = V(1,k) + epsil1*mdeltat*(Vup(k)-2*V(1,k)+V(2,k))/(deltax)^2 + mdeltat*karma_f(V(1,k),n(1,k),nB,M)/epsil2 ...
                    - mdeltat*cc_ext*iext(1,k) + mdeltat*cc_int*iint(1,k);
                Vnext(numpart) = V(numpart,k) + epsil1*mdeltat*(V(numpart-1,k)-2*V(numpart,k)+Vdn(k))/(deltax)^2 + mdeltat*karma_f(V(numpart,k),n(numpart,k),nB,M)/epsil2 ...
                    - mdeltat*cc_ext*iext(numpart,k) + mdeltat*cc_int*iint(numpart,k);
            elseif bc == 'p'
                %                 V(1,k+1) = V(1,k) + epsil1*deltat*(Vup(k)-2*V(1,k)+V(2,k))/(deltax)^2 + deltat*karma_f(V(1,k),n(1,k),nB,M)/epsil2 ...
                %                     - deltat*cc_ext*iext(1,k) + deltat*cc_int*iint(1,k);
            ...
            Vnext(2:(numpart-1)) = V(2:(numpart-1),k) + epsil1*mdeltat*(V(1:(numpart-2),k)-2*V(2:(numpart-1),k)+V(3:numpart,k))/(deltax)^2 + mdeltat*karma_f(V(2:(numpart-1),k),n(2:(numpart-1),k),nB,M)/epsil2 ...
                - mdeltat*cc_ext*iext(2:(numpart-1),k) + mdeltat*cc_int*iint(2:(numpart-1),k);
            ...
            Vnext(1) = V(1,k) + mdeltat*karma_f(V(1,k),n(1,k),nB,M)/epsil2 ...
                - mdeltat*cc_ext*iext(1,k) + mdeltat*cc_int*iint(1,k);
...
%        nnext = n(:,k) + deltat*karma_g(V(:,k),n(:,k))/tauN + un(:,k); % This one is consistent with b2=1 assumption
        nnext = n(:,k) + mdeltat*karma_g(V(:,k),n(:,k))/tauN - mdeltat*un(:,k); % This should be consistent with b2 = -0.05
