% Changed to enable multi-cell fiber simulations. Should be consistent with
% equations in karma_sim_statefbtest as of 10/18/09. Changed stimduration
% and epsil1 and presence/absence of acoeff*, hprime, etc. to depend on the
% number of cells. Changed function arguments to vectors instead of
% scalars. 
...
function [] = karma_sim_statefbtest(bcl,pertt,Vpertvec,npertvec)%karma_sim(karma_test,pubyear,stimperiod)
...
%    fiberlength = 1*deltax;%deltax*8;%deltax*2;%deltax*40;%deltax*3; %0.5%5%2.62; %0.3%deltax*3;% cm
    fiberlength = 2*deltax;%deltax*8;%deltax*2;%deltax*40;%deltax*3; %0.5%5%2.62; %0.3%deltax*3;% cm
...
load Aemp_2cell_b800_sti0p85_Vpert_0p03125_npert_1em4 Aemp
if 1%epsil1==0.009
...
%    desfile = 'steadystate_1cell_b800'; 
    desfile = 'steadystate_2cell_b800'; 
...
Vpertval = Vpertvec; % perturbation value in mV % 0.25/8 seems to give more accurate results than smaller values for bcl=800, perttime=14400.8. Could try even larger values to check accuracy
...
for ii=1:numrep % number of data writing cycles
    ...
    if ii == numrep
        ...
        if exist('Aemp')
         save data/configinfo s pert* B1 B2 stimperiod nB M epsil1 epsil2 tauN Vstar Vn Vh Vb b ch0 ch1 ch2 ch3 cc_* finaltime deltat deltax numstep numpart stimduration stimheight writeint numrep detvec fbon_time KIred1 stimindices iextd writeintsteps fiberlength stimlocindex dapd desfile *pert* capprox
        else
         save data/configinfo pert* B1 B2 stimperiod nB M epsil1 epsil2 tauN Vstar Vn Vh Vb b ch0 ch1 ch2 ch3 cc_* finaltime deltat deltax numstep numpart stimduration stimheight writeint numrep detvec fbon_time KIred1 stimindices iextd writeintsteps fiberlength stimlocindex dapd desfile *pert* capprox
       end
...
%            p2(ii)=plot(([((ii-1)*writeintsteps+1):10:ii*writeintsteps])*deltat,nerror([stimlocindex],1:10:writeintsteps),'r--');
        ...
        axis([14400 pertt+deltat+bcl -2*max([max(abs(Vpertval)) max(abs(npertval))]) 2*max([max(abs(Vpertval)) max(abs(npertval))])])
...
% BCL. 
            ...
            jve = (Verror(:,round((pertt-x*writeint+bcl)/deltat)+1) - s(1:numpart,round(pertt/deltat)+2))
jne = (nerror(:,round((pertt-x*writeint+bcl)/deltat)+1) - s((numpart+1):(2*numpart),round(pertt/deltat)+2))
...
% For karma_sim_jacobian_compare(800,14400.8,[0.25/16; 0],[0; 0])
% jvape =
%    0.12500000000000
%                   0
% jnape =
