% Plot results of gain tests. This version is intended for 2 cells and up,
% reflecting different gain ranges compared with the single-cell case.
clear variables
...
trajflag = 1; % use this flag to plot different gain ranges depending on the type of trajectory: 1=1to1, 2=2to1
...
for nn=1:numsub
    ...
    if nn==1
        ...
        elseif trajflag == 2
        gainrange = -0.35:0.05:0; % BCL = 160, 2 cells
        ...
        psflag = 0; % this is zero unless we want to use different symbols for different gain settings
...
%    bcl = 160; %ms
    ...
    for i=1:(size(allgains,3)-1)
        ...
        fname3 = ['edata_' num2str(numpart) 'cell_b' num2str(bcl) diffstr '_dt008_tauV0p7_tauN170_Vstar4_eig_relpert' epstr '_cdiff_Kp' gainstr{1}{1} '_Kpn' gainstr{2}{1}];
        if trajflag == 2
            fname3 = [fname3 '_2bcl'];
        end
        if ~exist([fname3 '.mat'])
        ...
        if nn == 1
            ...
            if psflag
                ...
                dtemp=diag(d);
                [maxabs,maxabsi]=max(abs(dtemp));
%                plot(L(1),dtemp(maxabsi),'*')
%                plot(L(1),abs(dtemp(maxabsi)),'*')
                plot(L(1),maxabs,'*')
%                plot(real(dtemp),imag(dtemp),'*')
dtemp
            end
...
ymax = 1.2;
elseif trajflag == 2
%    set(gcf,'Position',[192 483 491 495]) % smaller for ppt (Dell)
    set(gcf,'Position',[192 200 491 495]) % smaller for ppt (Toshiba)
xmin = -0.4;
xmax = 0.05;
ymin = 0;
ymax = .85;
...
if 0
    ...
    if numsub == 3
        ...
        set(gca,'Position',[0.13 0.13 0.72 0.77])
    end
    ...
    if numsub == 3
        ...
        %            set(gca,'Position',[0.13 0.16 0.775 0.315502])  %
        set(gca,'Position',[0.18 0.17 0.72 0.77]) % for unyces
    end
end
